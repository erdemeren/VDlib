...
/*-------------------------------------------------------------------*/
std::vector<int> cvx_hull::find_chull_vertices(
                                        std::vector<apf::Vector3>* vert_in) {

  ReadVertices(vert_in);
  DoubleTriangle();
  ConstructHull();
  EdgeOrderOnFaces();
  Print();
...
std::vector<int> cvx_hull::get_vert_id() {
  ...
  v= vertices;
  do {
    if( v->mark ) {ids.at(V) = v->vnum; V++; }
    v = v->next;
  } while ( v != vertices );
  return ids;
}

// Get the vert ids of the convex hull faces.
void cvx_hull::get_face_id(std::vector<std::vector<int> > * f_id) {
  tFace f = faces;
  int F = 0;
  ...
  do {
    f_id->push_back(std::vector<int>({f->vertex[0]->vnum,
                                      f->vertex[1]->vnum,
                                      f->vertex[2]->vnum}));
...
// Get the vert ids of the convex hull faces.
void cvx_hull::get_edge_id(std::vector<std::vector<int> > * e_id) {
  tEdge e = edges;
  int E = 0;
  ...
  do {
    e_id->push_back(std::vector<int>({e->endpts[0]->vnum,
                                      e->endpts[1]->vnum}));
    e = e->next;
  } while ( e != edges );
}
...
---------------------------------------------------------------------*/
tVertex	cvx_hull::MakeNullVertex( void )
{
   tVertex  v;
...
---------------------------------------------------------------------*/
void	cvx_hull::ReadVertices(std::vector<apf::Vector3>* vert_in) {
   tVertex  v;
   int	    vnum = 0;

  for(int i = 0; i < vert_in->size(); i++) {
    v = MakeNullVertex();
    v->v[X] = vert_in->at(i)[0];
    v->v[Y] = vert_in->at(i)[1];
    v->v[Z] = vert_in->at(i)[2];
    v->vnum = vnum++;
  }
}
...
void	cvx_hull::Print( void )
{
   ...
   do {           
      ...
      //a = norm_0(a);
      //b = norm_0(b);
      //a = a - vd_projpt(apf::Vector3(0,0,0), b, a);
      //a = norm_0(a);
      //if(vd_cross(a,b)[2] >= std::numeric_limits<double>::min() ) {
...
---------------------------------------------------------------------*/
void cvx_hull::SubVec( double a[3], double b[3], double c[3])
{
   int  i;
...
void cvx_hull::SubVec( double a[3], double b[3], apf::Vector3 & c)
{
   for(int i=0; i < 3; i++ )
      c[i] = a[i] - b[i];
...
/*---------------------------------------------------------------------
 ...
 are cleaned up. 
---------------------------------------------------------------------*/
void    cvx_hull::DoubleTriangle( void )
{
...
---------------------------------------------------------------------*/
void	cvx_hull::ConstructHull( void )
{
   tVertex  v, vnext;
   int 	    vol;
...
---------------------------------------------------------------------*/
bool 	cvx_hull::AddOne( tVertex p )
{
   tFace  f; 
   tEdge  e, temp;
   int 	  vol;
...
---------------------------------------------------------------------*/
int  cvx_hull::VolumeSign( tFace f, tVertex p )
{
  double  vol;
  int     voli;
  ...
  apf::Vector3 b(0,0,0);
  apf::Vector3 c(0,0,0);
  a[0] = f->vertex[0]->v[X] - p->v[X];
  a[1] = f->vertex[0]->v[Y] - p->v[Y];
  a[2] = f->vertex[0]->v[Z] - p->v[Z];
  b[0] = f->vertex[1]->v[X] - p->v[X];
  b[1] = f->vertex[1]->v[Y] - p->v[Y];
  b[2] = f->vertex[1]->v[Z] - p->v[Z];
  c[0] = f->vertex[2]->v[X] - p->v[X];
  c[1] = f->vertex[2]->v[Y] - p->v[Y];
  c[2] = f->vertex[2]->v[Z] - p->v[Z];

  c = c - vd_projpt(apf::Vector3(0,0,0), b, c);
  ...
  vol = a*(cross(b,c));
   ...
   if      ( vol >  std::numeric_limits<double>::min() )  return  1;
   else if ( vol < -std::numeric_limits<double>::min() )  return -1;
   else                    return  0;
...
---------------------------------------------------------------------*/
int  cvx_hull::Volumei( tFace f, tVertex p )
{
   int  vol;
...
/*-------------------------------------------------------------------*/
void	cvx_hull::PrintPoint( tVertex p )
{
   int	i;
...
the new face.
---------------------------------------------------------------------*/
tFace	cvx_hull::MakeConeFace( tEdge e, tVertex p )
{
   tEdge  new_edge[2];
   tFace  new_face;
   int 	  i, j;
...
---------------------------------------------------------------------*/
void	cvx_hull::MakeCcw( tFace f, tEdge e, tVertex p )
{
...
---------------------------------------------------------------------*/
tEdge 	cvx_hull::MakeNullEdge( void )
{
   tEdge  e;
...
to the empty cell.
---------------------------------------------------------------------*/
tFace 	cvx_hull::MakeNullFace( void )
{
   tFace  f;
   int    i;
...
---------------------------------------------------------------------*/
tFace   cvx_hull::MakeFace( tVertex v0, tVertex v1, tVertex v2, tFace fold )
{
   tFace  f;
   tEdge  e0, e1, e2;
...
---------------------------------------------------------------------*/
void	cvx_hull::CleanUp( tVertex *pvnext )
{
   CleanEdges();
   CleanFaces();
...
---------------------------------------------------------------------*/
void	cvx_hull::CleanVertices( tVertex *pvnext )
{
   tEdge    e;
   tVertex  v, t;
...
---------------------------------------------------------------------*/
bool	cvx_hull::Collinear( tVertex a, tVertex b, tVertex c )
{
  apf::Vector3 v1(0,0,0);
  apf::Vector3 v2(0,0,0);
  v1[0] = b->v[X] - a->v[X]; v1[1] = b->v[Y] - a->v[Y];
  v1[2] = b->v[Z] - a->v[Z];
  v2[0] = c->v[X] - a->v[X]; v2[1] = c->v[Y] - a->v[Y];
  v2[2] = c->v[Z] - a->v[Z];

  v1 = norm_0(v1);
  v2 = norm_0(v2);
  double proj = std::fabs(std::fabs(v1*v2) - 1);
  return proj < std::numeric_limits<double>::min();
/*
   return 
...
---------------------------------------------------------------------*/
void	cvx_hull::Consistency( void )
{
   register tEdge  e;
   register int    i, j;
...
---------------------------------------------------------------------*/
void	cvx_hull::CheckEuler( int V, int E, int F )
{
   if ( check )
...
=====================================================================*/
/*-------------------------------------------------------------------*/
void	cvx_hull::PrintOut( tVertex v )
{
   fprintf( stderr, "\nHead vertex %d = %6x :\n", v->vnum, v );
   PrintVertices();
   PrintEdges();
...
/*-------------------------------------------------------------------*/
void	cvx_hull::PrintVertices( void )
{
   tVertex  temp;
...
the face.
-------------------------------------------------------------------*/
void	cvx_hull::CheckEndpts ( void )
{
   int 	   i;
   tFace   fstart;
   tEdge   e;
...
------------------------------------------------------------------*/
void    cvx_hull::EdgeOrderOnFaces ( void ) {
  tFace f = faces;
  tEdge e_new;
  int i,j;
