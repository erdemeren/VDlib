...
void solv_lin::numb_vert() {
  ...
  int max_gn = 0;
  for (int p_id = 0; p_id < PCU_Comm_Peers(); ++p_id) {
    if (p_id == PCU_Comm_Self()) {
      apf::MeshIterator *vertIter = m->begin(0);
      apf::MeshEntity *vert = nullptr;
      while ((vert = m->iterate(vertIter))) {
        if (m->isOwned(vert) && m->getOwner(vert) == p_id) {
          const auto numb = apf::getNumber(gn, vert, 0);
          if(numb > max_gn)
            max_gn = numb;
        }
      }
...
void solv_lin::collect_matrix(apf::Mesh2* m_in) {
  A = gsl_spmatrix_alloc(n, n); /* triplet format */
  C;                            /* compressed format */

  f = gsl_vector_alloc(n);        /* right hand side vector */
  u = gsl_vector_alloc(n);        /* solution vector */
  size_t i;
...
void solv_lin::init(apf::Mesh2* m_in) {
  m = m_in;
  numb_vert();
...
double solv_lin::solve(const double tol = 1.0e-6) {
...
solv_lin::solv_lin(apf::Mesh2* m_in) {
  init(m_in);
}
