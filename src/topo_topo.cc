...
cell_base::cell_base(int n0, int n1, int n2, int n3) : 
  con(3, std::vector<std::vector<int> > (0, std::vector<int>(0))), 
  con_up(3, std::vector<std::vector<int> > (0, std::vector<int>(0))), 
  ...
  fix_list(0)
 {
  ...
  for(int dim = 1; dim < 4; dim++) {
    con.at(dim-1).resize(n[dim]);
  }
  for(int dim = 0; dim < 3; dim++) {
    con_up.at(dim).resize(n[dim]);
  }
...
int cell_base::cell_sz(int dim, int tag) {
  ...
  if (dim < 1 or dim > 3)
    return -1;
  return con.at(dim-1).at(tag).size();
}
...
// set_conn_up is called through this only. 
bool cell_base::set_conn(int dim, int tag, struct ent_conn* e_con) {
  ...
  //std::cout << std::endl;
  con.at(dim - 1).at(tag) = e_con->conn;
  ent_conn* e_up = new ent_conn();
  for (int i = 0; i < e_con->conn.size(); i++) {
    int id_curr = e_con->conn.at(i);
    get_conn_up(dim - 1, id_curr, e_up);
    if(!e_up->chk_ent(tag))
      e_up->add_ent(tag);
...
bool cell_base::set_conn_repl(int dim, int tag_new, int tag_old, struct ent_conn* e_con) {
  ...
  if (dim > 3 or dim < 1 or tag_new > n[dim])
    return false;
  ...
  //  std::cout << dim - 1 << "c" << e_con->conn.at(i) + 1 << " ";
  //std::cout << std::endl;
  con.at(dim - 1).at(tag_new) = e_con->conn;
  ent_conn* e_up = new ent_conn();
  for (int i = 0; i < e_con->conn.size(); i++) {
    int id_curr = e_con->conn.at(i);
    get_conn_up(dim - 1, id_curr, e_up);
    e_up->repl_tag(tag_old, tag_new);
    set_conn_up(dim - 1, id_curr, e_up);
  }
  delete e_up;
}
...
bool cell_base::get_conn(int dim, int tag, struct ent_conn* e_con) {
  ...
  if (dim > 3 or dim < 1 or tag > n[dim]) {
    ...
    e_con->conn = con.at(dim - 1).at(tag);
    return true;
  }
}

// Get the connection list of a given entity. 
bool cell_base::get_conn_up(int dim, int tag, struct ent_conn* e_con) {
  if (dim > 2 or dim < 0 or tag > n[dim]) {
    e_con->resize(0);
    return false;
  }
  else {
    e_con->conn = con_up.at(dim).at(tag);
    return true;
  }
}
...
bool cell_base::get_conn_gmi(int dim, int tag, struct ent_conn* e_con) {
  ...
  bool flag = get_conn(dim, tag-1, e_con);
...
// Get the given lower dimensional connection list of a given entity. 
bool cell_base::get_conn_upper(int dim_adj, int dim, int tag, 
                                                struct ent_conn* e_cover) {
  ...
  if(dim_adj == dim+1) {
    get_conn_up(dim, tag, e_cover);
  }
  else {
    get_conn_up(dim, tag, &e_con);

    for (int i = 0; i < e_con.conn.size(); i++) {
      get_conn_upper(dim_adj, dim+1, e_con.conn.at(i), &e_next); 
      for (int j = 0; j < e_next.conn.size(); j++) {
        if(!e_cover->chk_ent(e_next.conn.at(j))) {
          e_cover->add_ent(e_next.conn.at(j));
        }
...
bool cell_base::get_conn_lower_gmi(int dim_adj, int dim, struct ent_conn* e_set,
                                                struct ent_conn* e_cover) {
  for (int i = 0; i < e_set->conn.size(); i++)
    e_set->conn.at(i)--;
  bool flag = get_conn_lower(dim_adj, dim, e_set, e_cover);
...
// Get the given lower dimensional connection list of a given entity. 
bool cell_base::get_conn_upper(int dim_adj, int dim, struct ent_conn* e_set,
                                                struct ent_conn* e_cover) {
...
bool cell_base::get_conn_dim(int dim_adj, int dim, int tag, 
  ...
  // dim_adj > dim.
    ...
    get_conn_upper(dim_adj, dim, tag, e_cover);
  }
  ...
  if (!is_free(dim, tag)) {
    ...
    if (get_sz(dim-1) > 0) {
      ...
      for (int i = 0; i < e_down.conn.size(); i++) {
        get_conn_up(dim-1, e_down.conn.at(i), &e_con);
        while (e_con.chk_ent(tag)) {
          e_con.rem_ent(tag);
        }
        // In any case, shift indices. Not anymore.
        // e_con.shift_ind(tag);
        set_conn_up(dim-1, e_down.conn.at(i), &e_con);
      }
    }

    struct ent_conn e_empty; 
    set_conn(dim, tag, &e_empty);
    set_conn_up(dim, tag, &e_empty);
    if(dim < 3)
...
bool cell_base::repl_cell(int dim, int tag_new, int tag_old) {
  ...
  for (int i = 0; i < e_adj.conn.size(); i++) {
    get_conn(dim+1, e_adj.conn.at(i), &e_con);
    assert (e_con.chk_ent(tag_old));
    if (e_con.chk_ent(tag_new)) { // tag1 does exist.
      e_con.rem_ent(tag_old);
    }
    else {
      e_con.repl_tag(tag_new, tag_old);
    }
    set_conn(dim+1, e_adj.conn.at(i), &e_con);
  }
  get_conn(dim, tag_new, &e_adj);
  for (int i = 0; i < e_adj.conn.size(); i++) {
    get_conn_up(dim-1, e_adj.conn.at(i), &e_con);
    assert (e_con.chk_ent(tag_old));
    if (e_con.chk_ent(tag_new)) { // tag1 does exist.
      e_con.rem_ent(tag_old);
    }
    ...
    set_conn_up(dim-1, e_adj.conn.at(i), &e_con);
  }
  ...
  //    // In any case, shift indices. Not anymore.
  //    // e_con.shift_ind(tag);
  //    set_conn_repl(dim+1, tag_new, tag_old, &e_con);
  //  }
  //}
}
...
bool cell_base::add_free(int dim, int add_sz) {
  ...
  int cont_sz = 0;
  if(dim > 0)
    con.at(dim - 1).resize(con.at(dim - 1).size()+add_sz);
  if(dim < 3)
    con_up.at(dim).resize(con_up.at(dim).size()+add_sz);
  ...
  if (dim == 1) {
    ...
    if (get_conn(dim, tag, &e_con)) {
      ...
      if (e_con.conn.size() > 0) {
        ...
        for (int i = 0; i < e_con.conn.size(); i++) {
          if(tag_0cell != e_con.conn.at(i)) {
            repl_cell(0, tag_0cell, e_con.conn.at(i));
            rem_cell(0, e_con.conn.at(i));
          }
        }
/*
        if(tag_0cell != e_con.conn.at(0)) {
          get_conn_up(0, e_con.conn.at(0), &e_con_1c);
...
// Keep the values in v_keep, remove the rest from v_mod.
void cell_base::keep_vect_vect(std::vector<int> &v_mod, std::vector<int> const &v_keep) {
  int total = v_mod.size();
  int rmv_count = 0;
  for (int j = 0; j < v_keep.size(); j++) {
    int tag_curr = v_keep.at(j);
    std::vector<int>::iterator it = std::find(v_mod.begin(), v_mod.end(), tag_curr);
    if(it == v_mod.end()) {
      int tag_temp = v_mod.at(j);
      v_mod.at(j) = v_mod.back();
      v_mod.at(total - 1 - rmv_count) = tag_temp;
      j = j - 1;
      rmv_count = rmv_count + 1;
    }
  }
  std::sort (v_mod.begin(), v_mod.end());
}
...
void cell_base::write_dot(const char* dotname) {
...
cell_base& cell_base::operator=( const cell_base& other ) {
  ...
  for(int dim = 0; dim < 3; dim++) {
    for(int i = 0; i < other.con_up.at(dim).size(); i++) {
      con_up.at(dim).at(i) = other.con_up.at(dim).at(i);
    }
    for(int i = 0; i < other.con.at(dim).size(); i++) {
      con.at(dim).at(i) = other.con.at(dim).at(i);
    }
  }
...
cell_base& cell_base::copy_local(cell_base& other, int c0_id ) {
  ...
  for (int i = 0; i < c_3.conn.size(); i++) {
    ...
    temp = other.con.at(2).at(c_curr);
    keep_vect_vect(temp, c_2.conn);
    con.at(2).at(c_curr) = temp;
  }
  ...
  for (int i = 0; i < c_2.conn.size(); i++) {
    ...
    temp = other.con.at(1).at(c_curr);
    keep_vect_vect(temp, c_1.conn);
    con.at(1).at(c_curr) = temp;

    temp = other.con_up.at(2).at(c_curr);
    keep_vect_vect(temp, c_3.conn);
    con_up.at(2).at(c_curr) = temp;
  }
  ...
  for (int i = 0; i < c_1.conn.size(); i++) {
    ...
    temp = other.con.at(0).at(c_curr);
    keep_vect_vect(temp, c_0.conn);
    con.at(0).at(c_curr) = temp;

    temp = other.con_up.at(1).at(c_curr);
    keep_vect_vect(temp, c_2.conn);
    con_up.at(1).at(c_curr) = temp;
  }
  ...
  for (int i = 0; i < c_1.conn.size(); i++) {
    ...
    temp = other.con_up.at(0).at(c_curr);
    keep_vect_vect(temp, c_1.conn);
    con_up.at(0).at(c_curr) = temp;
  }
